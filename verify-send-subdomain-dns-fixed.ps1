...
try {
    ...
    if (-not $spfFound) {
        Write-Host "   X SPF Record NOT found" -ForegroundColor Red
    ...
    Write-Host "   X No TXT records found for send.asset-tracer.com" -ForegroundColor Red
...
try {
    ...
    if (-not $mxFound) {
        Write-Host "   X Correct MX Record NOT found" -ForegroundColor Red
    ...
    Write-Host "   X No MX records found for send.asset-tracer.com" -ForegroundColor Red
    Write-Host "   Error: $($_.Exception.Message)" -ForegroundColor Gray
}
...
Write-Host "If you see X, add the missing records in Namecheap." -ForegroundColor Yellow
Write-Host ""
